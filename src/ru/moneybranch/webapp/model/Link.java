...
public class Link {
    ...
    private int i; // Разница между инстанц и локал переменной. Данная переменная, которая используется циклом, будет создана в куче, для каждого экземпляра, где-то далеко в памяти (print())
    ...
    /**
     * Но объявляя переменную локальной, она создается в регистрах. Ей не выделяется отдельная память. Т.к. локальные переменные быстро появляются (в регистрах), и выполнив свою работу, исчезают
     * Поэтому, создавать поля, и использовать их в локальных конструкциях - плохой тон
     */
    public void print(){
        for (int i = 0; i<5; i++){
    ...
    // Однако, если необходимо сравнение объектов, и получение положительного результата, при одинаковых свойствах. ТО НУЖНО И ПЕРЕОПРЕДЕЛИТЬ этот метод equals
    @Override
